...
{
    remove_id("db/cats.csv", $rem_id);
    header("Location: /a_cats_list.php");
}
?>
    ...
    while (($arr = fgetcsv($fd, 0, ";")) !== FALSE)
    {
        if (isset($arr) && isset($arr[0]) && is_numeric($arr[0]))
        {
            echo "\n\t<div class=\"cat_list\">\n
                ...
                \t\t<a href=\"/a_items_list.php?cat_id=".$arr[0]."\">".$arr[1]."</a>\n
                \t\t<a href=\"".$_SERVER['REQUEST_URI']."?rem_id=".$arr[0]."\"><img src=\"img/adm/remove.png\" alt=\"Remove\" class=\"pic\"></a>\n
                \t\t<a href=\"/a_cat_edit.php?cat_id=".$arr[0]."\"><img src=\"img/adm/edit.png\" alt=\"Edit\" class=\"pic\"></a>\n
                \t</div>";
        }
    }
    fclose($fd);
